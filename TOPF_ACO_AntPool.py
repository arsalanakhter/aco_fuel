...
class TOPF_ACO_AntPool:
    ...
    def feasible_for(self, ant):
        ...
        # TODO: Check if this condition forces a depot to not be
        #  visited by multiple ants - Can reach node and then depot
        #  with fuel left
        feasible_neighbours = [idx for idx, i in enumerate(
            self.visited_g) if i != 1]
        # Remove first node, as we do not want the first node to be a
        # feasible neighbour for ant to go directly. If there are no
        # feasible neighbours, we'll send the ant to the starting
        # location manually.
        feasible_neighbours.remove(0)
        # Based on this ant's fuel, remove those nodes from feasible
        ...
        # Also remove those neighbours from which the ant cannot
        ...
        # this neighbour. Assuming start node is the first node in
        # the node list
        for n in feasible_neighbours:
    ...
    def compute_paths(self, rng, pheromone_matrix):
        # for each ant that is not done calculate the feasible nodes
        # if there are feasible nodes, move the ant, and mark that
        # node as visited if not, move ant deterministically to the
        # starting location, ensuring that the move is feasible. Then
        # mark the ant done.
        paths = []
        no_of_ants_done = 0
